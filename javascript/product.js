...
function load_product() {
	makeRequest('action=product&product_title='+product_title+'&product_extend='+product_extend+'&product_catid='+product_catid+'&product_pid='+product_pid, AJPath, 'into_product');
}
function into_product() {
	if(xmlHttp.readyState==4 && xmlHttp.status==200) {
		$('load_product').innerHTML = xmlHttp.responseText;
		if(xmlHttp.responseText == '') {
			$('pid').value = product_pid = 0;
			$('load_product_option').innerHTML = xmlHttp.responseText;
			Dh('load_product_option_tr');
		}
	}
}
