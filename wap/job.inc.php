...
if($resume) {
	...
	if($itemid) {
		$item = $db->get_one("SELECT * FROM {$table} WHERE itemid=$itemid AND status=3 AND open=3");
		$item or wap_msg('信息不存在');
		extract($item);

		$CAT = get_cat($catid);
		if(!check_group($_groupid, $MOD['group_show_resume'])) wap_msg('无权浏览的页面');
		$description = '';
		$user_status = 3;
		...
		if($kw) {
			check_group($_groupid, $MOD['group_search_resume']) or wap_msg('无权搜索');
		}
