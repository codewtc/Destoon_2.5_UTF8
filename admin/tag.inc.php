...
switch($action) {
	...
	case 'cat':
		...
		$mid or exit();
		if($mid < 8) $mid = 1;
		echo category_select('catids', '不限分类', 0, $mid, 'onchange="stoinp(this.value, \'catid\', \',\');"');
		exit;
	break;
	case 'preview':
		$db->halt = 0;
	...
	default:
		...
		$out = array('ad', 'ad_place', 'admin', 'area', 'ask' ,'category', 'favorite', 'finance_cash', 'finance_charge', 'finance_record', 'finance_trade', 'friend', 'group', 'guestbook', 'keylink', 'log', 'mail', 'mail_list', 'message', 'module', 'session', 'style', 'type', 'vip');
