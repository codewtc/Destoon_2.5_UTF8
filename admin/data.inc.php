...
switch($action) {
	case 'config':
		if($submit) {
			...
			if($code) {
				file_put(DT_ROOT.'/file/data/'.$data['name'].'.inc.php', '<?php defined(\'IN_DESTOON\') or exit(\'Access Denied\');'.stripslashes($code).' ?>');
			} else {
				@unlink(DT_ROOT.'/file/data/'.$data['name'].'.inc.php');
			}
	...
	case 'delete':
		 if($name) {
			@unlink(DT_ROOT.'/file/data/'.$name.'.php');
			@unlink(DT_ROOT.'/file/data/'.$name.'.inc.php');
		}
		dmsg('删除成功', '?file='.$file);
	break;
	case 'view':
		...
		while($F = $sc->fetch_array($result)) {
			$T = array();
			if($db_charset) $F = convert($F, $db_charset, $CFG['charset']);
			...
			if($sc->affected_rows($result)) {
				while($F = $sc->fetch_array($result)) {
					...
					if($type == 0) {
						...
						$db->query("INSERT INTO {$table_company_data} (userid,content)  VALUES ('$userid', '$content')");
					} else if($type == 2) {
						$sqlk = $sqlv = '';
						foreach($T as $k=>$v) {
							$sqlk .= ','.$k; $sqlv .= ",'$v'";
