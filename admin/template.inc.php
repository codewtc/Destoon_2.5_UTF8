...
isset($dir) or $dir = '';
$menus = array (
	array('新建模板', '?file=template&action=add&dir='.$dir),
    array('模板管理', '?file=template'),
    array('重建缓存', '?file=template&action=cache'),
);
...
switch($action) {
	...
	case 'preview':
		...
		$head_title = '模板预览';
		include CE_ROOT.'/tpl/_preview.tpl.php';
		exit();
	break;
	case 'import':
		$fileid or msg();
	...
	case 'template_name':
		$fileid or exit('0');
		$name or exit('0');
		if(strtolower($CFG['charset']) != 'utf-8') $name = convert($name, 'utf-8', $CFG['charset']);
