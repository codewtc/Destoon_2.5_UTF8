...
<tr>
<td colspan="2"><input type="checkbox" name="backup" value="1"/> 保存时，创建一个备份文件&nbsp;&nbsp;<input type="submit" name="submit" value=" 保 存 " class="btn"/>&nbsp;&nbsp;<input type="button" value=" 预 览 " class="btn" onclick="Preview();"/>&nbsp;&nbsp;<input type="button" value=" 返 回 " class="btn" onclick="window.history.go(-1);"/>&nbsp;&nbsp;<input type="reset" value=" 重 置 " class="btn"/>&nbsp;&nbsp;<input type="button" value=" 缩 小 " class="btn" onclick="Zoom('-');"/>&nbsp;&nbsp;<input type="button" value=" 放 大 " class="btn" onclick="Zoom('+');"/></td>
</tr>
</table>
</form>
<br/>
...
function Zoom(t) {
	var h = $('content').style.height ? $('content').style.height : '300px';
	var n = Number(h.replace('px', ''));
	n = t == '+' ? n+100 : n-100;
	if(n > 100) $('content').style.height = n+'px';
}
function Preview() {
	if($('content').value == '') {
		Dtip('模板内容为空');
	} else {
		$('pcontent').value = $('content').value;
		$('p').submit();
	}
}
</script>
