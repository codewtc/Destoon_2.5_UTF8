...
switch($action) {
	...
	case 'count':
		...
		msg('ID从'.$fid.'至'.($catid-1).'更新成功', "?mid=$mid&file=$file&action=$action&fid=$catid&tid=$tid&num=$num");
	break;
	case 'ckdir':
		...
		if(is_array($catids)) {
			foreach($catids as $catid) {
				if(isset($this->category[$catid])) {
					...
					if($arrchildid) $this->db->query("DELETE FROM {$this->table} WHERE catid IN ($arrchildid)");
				}
			}
		} else {
			$catid = $catids;
