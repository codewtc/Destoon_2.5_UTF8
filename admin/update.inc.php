...
switch($action) {
	case 'download':
		...
		$url = 'http://www.destoon.com/update.php?product=b2b&release='.$release.'&charset='.$CFG['charset'];
		$code = @file_get($url);
		if($code) {
	...
	break;
	case 'unzip':
		require DT_ROOT.'/include/unzip.class.php';
		$zip = new unzip;
	...
	case 'copy':
		...
		foreach($files as $v) {
			...
			file_copy($v, $file_a) or msg('无法覆盖'.str_replace(DT_ROOT.'/', '', $file_a).'<br/>请设置此文件及上级目录属性为'.DT_CHMOD.'，然后刷新此页');
		}
