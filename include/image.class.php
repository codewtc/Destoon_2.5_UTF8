...
class image {
	...
	function image($g_img) {
		...
		$this->w_img = DT_ROOT.'/'.($DT['water_image'] ? $DT['water_image'] : 'file/watermark.png');
		...
		$this->text_font = DT_ROOT.'/file/'.($DT['water_font'] ? $DT['water_font'] : 'simhei.ttf');
    ...
    function watertext($save_name = '') {
		...
		if(strtolower($CFG['charset']) != 'utf-8') {
			$this->text = convert($this->text, $CFG['charset'], 'utf-8');
		}
		$this->save_name = $save_name ? $save_name : $this->g_img;
	...
	function thumb($w = 0, $h = 0, $save_name = '') {
		if(!$this->make) return false;
    ...
    function save() {
        switch($this->g_type) {
            ...
            case '2': imagejpeg($this->img, $this->save_name, $this->jpeg_quality); break;
