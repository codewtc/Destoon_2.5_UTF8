...
switch($action) {
	case 'login':
		...
		$user = $db->get_one("SELECT username,passport,password,groupid,email FROM {$DT_PRE}member WHERE username='$username' limit 0,1");
		if( $uid == -1 ) {/* Ucenter 用户不存在或被删除 */
			...
			if($user) {
				...
				$auth = urlencode(dcrypt($username.'|'.$rt_password.'|'.$rt_email));
