...
switch($action) {
	...
	case 'show':
		...
		msg('ID从'.$fid.'至'.($itemid-1).$MOD['name'].($update ? '更新' : '生成').'成功', "?moduleid=$moduleid&file=$file&action=$action&fid=$itemid&tid=$tid&num=$num&update=$update&all=$all&one=$one");
	break;
	case 'cate':
		$catid or msg('请选择分类');
		if(!isset($item_count)) {
			$condition = $CATEGORY[$catid]['child'] ? "catid IN (".$CATEGORY[$catid]['arrchildid'].")" : "catid=$catid";
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $condition AND status=3");
			$item_count = $r['num'];
		}
		isset($num) or $num = 50;
		isset($fid) or $fid = 1;
