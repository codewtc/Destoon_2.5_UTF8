...
$MG['quote_limit'] > -1 or dalert('您所在的会员组没有权限使用此功能，请升级', 'goback');
...
switch($action) {
	case 'add':
		if($MG['quote_limit'] && $limit_used >= $MG['quote_limit']) dalert('最多可发布'.$MG['quote_limit'].'条'.$MOD['name'].' 当前已发布'.$limit_used.'条', $_userid ? $MODULE[2]['linkurl'].$DT['file_my'].'?mid='.$mid : $MODULE[2]['linkurl'].$DT['file_my']);
		if($MG['day_limit']) {
		...
		if($MG['quote_free_limit'] > 0) {
			$fee_add = ($MOD['fee_add'] && !$MG['fee_mode'] && $limit_used >= $MG['quote_free_limit'] && $_userid) ? dround($MOD['fee_add']) : 0;
		} else {
			$fee_add = 0;
		}
