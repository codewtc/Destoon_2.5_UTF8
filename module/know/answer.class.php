...
class answer {
	...
	function pass($post) {
		...
		if(!$post['content']) return $this->_('请填写答案内容');
		return true;
	}
		...
		if(is_array($itemid)) {
			...
			if($r) {
				...
				if($r['username'] && $MOD['credit_del_answer']) {
					...
					credit_record($r['username'], -$MOD['credit_del_answer'], 'system', '答案删除', 'ID:'.$r['qid']);
				}
			}
		}
	}
		...
		if(is_array($itemid)) {
			...
			if($MOD['credit_answer'] && $status == 3) {
				...
				if($item['username']) {
					...
					credit_record($item['username'], $MOD['credit_answer'], 'system', '回答问题', 'ID:'.$itemid);
				}
			}
