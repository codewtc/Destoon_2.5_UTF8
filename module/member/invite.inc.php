	...
	if(isset($user) && preg_match("/^[a-z0-9]{2,}$/", $user)) {
		...
		if($c) {
			...
			if($could_credit && $MOD['credit_maxip'] > 0) {
				$r = $db->get_one("SELECT SUM(amount) AS total FROM {$DT_PRE}finance_credit WHERE username='$username' AND addtime>$DT_TIME-86400 AND reason='积分推广'");
				if($r['total'] > $MOD['credit_maxip']) $could_credit = false;
			}
			if($could_credit) {
				credit_add($username, $MOD['credit_ip']);
				credit_record($username, $MOD['credit_ip'], 'system', '积分推广', $DT_IP);
				set_cookie('inviter', dcrypt($username), $DT_TIME+86400*365);
			}
		} else {
			dheader(DT_PATH);
		}
