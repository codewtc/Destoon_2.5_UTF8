...
switch($action) {
	...
	default:
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}ask WHERE 1 $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$asks = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}ask WHERE 1 $condition ORDER BY $dorder[$order] LIMIT $offset,$pagesize");
