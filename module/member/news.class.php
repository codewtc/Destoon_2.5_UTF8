...
class news {
    ...
    function news() {
		...
		$this->db = &$db;
		$this->fields = array('title','typeid','style','status','username','addtime','editor','edittime','linkurl','note');
    }
	...
	function add($post) {
		...
		if($post['username'] && $MOD['credit_add_news']) {
			...
			credit_record($post['username'], $MOD['credit_add_news'], 'system', '新闻发布', 'ID:'.$this->itemid);
		}
		return $this->itemid;
	}
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($r['username'] && $MOD['credit_del_news']) {
				...
				credit_record($r['username'], -$MOD['credit_del_news'], 'system', '新闻删除', 'ID:'.$this->itemid);
			}
		}
	}
