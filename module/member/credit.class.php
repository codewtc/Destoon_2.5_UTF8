...
class credit {
	...
	function pass($post) {
		...
		if($post['totime']) {
			if(!is_date($post['totime'])) return $this->_('请选择证书到期时间');
	...
	function get_list($condition = 'status=3', $order = 'addtime DESC') {
		...
		while($r = $this->db->fetch_array($result)) {
			...
			$r['todate'] = $r['totime'] ? timetodate($r['totime'], 3) : '永久';
	...
	function add($post) {
		...
		if($post['username'] && $MOD['credit_add_credit']) {
			...
			credit_record($post['username'], $MOD['credit_add_credit'], 'system', '证书上传', 'ID:'.$this->itemid);
		}
		return $this->itemid;
	}
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($r['username'] && $MOD['credit_del_credit']) {
				...
				credit_record($r['username'], -$MOD['credit_del_credit'], 'system', '证书删除', 'ID:'.$this->itemid);
			}
		}
	}
