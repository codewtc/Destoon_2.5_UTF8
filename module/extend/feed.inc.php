...
if($mid && $mid > 4 && isset($MODULE[$mid]) && !$MODULE[$mid]['islink']) {
	...
	$category_select = category_select('catid', '分类', $catid, $moduleid);
	if(in_array($MODULE[$mid]['module'], array('sell','buy', 'exhibit', 'info', 'job'))) $area_select = ajax_area_select('areaid', '选择地区', $areaid);
}
