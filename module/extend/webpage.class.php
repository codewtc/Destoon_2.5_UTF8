...
class webpage {
    ...
    function webpage() {
		...
		$this->db = &$db;
		$this->fields = array('item', 'title','level','style','content','seo_title','seo_keywords','seo_description','editor','edittime','template', 'islink', 'linkurl');
    }

	function pass($post) {
		if(!is_array($post)) return false;
		if(!$post['title']) return $this->_('请填写单页标题');
		if(isset($post['islink'])) {
			if(!$post['linkurl']) return $this->_('请填写链接地址');
		} else {
			if(!$post['content']) return $this->_('请填写单页内容');
			if($post['filepath'] && !preg_match("/^[0-9a-z_\-\/]+\/$/", $post['filepath'])) return $this->_('文件路径格式有误');
			if($post['filename'] && !preg_match("/^[0-9a-z_\-]+\.[a-z]+$/", $post['filename'])) return $this->_('文件名格式有误');
			if($post['filename']) {
				if($this->itemid) {
					$r = $this->get_one();
					if($r['linkurl'] != $post['filepath'].$post['filename']) {
						if(is_file(DT_ROOT.'/'.$post['filepath'].$post['filename'])) return $this->_('指定路径文件已存在');
					}
				} else {
					if(is_file(DT_ROOT.'/'.$post['filepath'].$post['filename'])) return $this->_('指定路径文件已存在');
				}
