...
class vote {
    ...
    function vote() {
		...
		$this->db = &$db;
		$this->fields = array('typeid', 'title','style','linkto','content','addtime','fromtime','totime','editor','edittime','template_vote','template', 'vote_min', 'vote_max', 'linkurl', 'choose', 's1', 's2', 's3', 's4', 's5', 's6', 's7', 's8', 's9', 's10');
    }

	function pass($post) {
		if(!is_array($post)) return false;
		if(!$post['typeid']) return $this->_('请选择投票分类');
		if(!$post['title']) return $this->_('请填写投票标题');
		return true;
	}
	...
	function delete($itemid) {
		if(is_array($itemid)) {
			...
			unlink(CE_ROOT.'/htm/vote_'.$r['itemid'].'.htm');
		}
	}
	...
	function html() {
		...
		while($r = $this->db->fetch_array($result)) {
			$itemid = $r['itemid'];
			tohtml('vote', $module, "itemid=$itemid");
		}
		return true;
	}
